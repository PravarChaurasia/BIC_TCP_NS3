...
TcpBic::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpBic")
    .SetParent<TcpCongestionOps> ()       //API to set
...
TcpBic::TcpBic ()        // constructor
  : TcpCongestionOps (),  // API 
    m_cWndCnt (0),
    m_lastMaxCwnd (0),
    m_lastCwnd (0),
    m_epochStart (Time::Min ())
        // delayed_ack in not present as in linux
{
  NS_LOG_FUNCTION (this);  // current object_ref --> this
}

TcpBic::TcpBic (const TcpBic &sock)   //bic_init()
...
//cnt = (cnt << ACK_RATIO_SHIFT) / ca->delayed_ack;  in_linux but since no delayed_ack in ns3
  if (cnt == 0)
    {
      cnt = 1;
    }
